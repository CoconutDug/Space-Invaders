...
void D3DGraphics::DrawSurface(Color key, int posX, int posY)
{
	for (int y = 0; y < (int)sysBuffer.GetHeight(); y++)
	{
		for (int x = 0; x < (int)sysBuffer.GetWidth(); x++)
		{
			if (key != GetPixel(x, y))
			{
				PutPixel(x + posX, y + posY, GetPixel(x, y));
			}
		}
	}
}
