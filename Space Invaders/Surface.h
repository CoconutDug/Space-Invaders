...
public:
	...
	static std::vector<Surface> VectorFromFile(const std::wstring& basename, int nSurfaces)
	{
		std::vector<Surface> vSurface;
		for (int index = 0; index < nSurfaces; index++)
		{
			std::wstringstream ss;
			ss << std::setw(4) << std::setfill(L'0') << index;
			vSurface.push_back(FromFile(basename + ss.str() + std::wstring(L".bmp")));
		}
		return vSurface;
	}
